...
module.exports = context => async (data, functionContext) => {
   ...
   * 5. Create a `vaultDoc` object by chaining off of `admin.firestore().collection('user-owned').
   * 6. Return `vaultDoc.set({encrypted, encryptedDate: Date.now()}, { merge: true })`
   ...
   * We're going to set that string as the `encrypted` attribute of our `vaultDoc`.
   * We could use `vaultDoc.update(updates)` or `vaultDoc.set(updates, { merge: true })`.
   * The second `{ merge: true }` argument turns `vaultDoc.set` into an upsert. The upsert is
   * great for testing, because we don't have to see the `vaultDoc` with data to run an upsert.
   * `vaultDoc.update` will fail if a record does not already exist.
   *
  ...
  const encrypted = await encryptSecretWithPassword({ password, secret });
  const vaultDoc = admin
    .firestore()
  ...
  return vaultDoc.set({ encrypted, encryptedDate: Date.now() }, { merge: true });
};
